...
/*
 * Author Majid Khosravi on 31/05/2021.
 * Farakav.com
...
public class CountDownByEmptyView extends ConstraintLayout implements BaseCountDown, View.OnClickListener {
    ...
    private RemainTimeByEmptyViewBinding mBinding;
    private CountDownTimer mCountDownTimer;
    private TimerState mTimerStateListener;

    public CountDownByEmptyView(Context context) {
        this(context, null);
    }

    public CountDownByEmptyView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CountDownByEmptyView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        mContext = context;

        titleText = getResources().getString(R.string.text_title);
        messageText = getResources().getString(R.string.text_message);
    ...
    private void initByAttributes(TypedArray attributes) {
        mBinding = DataBindingUtil.inflate(LayoutInflater.from(mContext), R.layout.remain_time_by_empty_view, this, true);
        if (attributes.getString(R.styleable.CountDownView_title) != null)
            titleText = attributes.getString(R.styleable.CountDownView_title);
        if (attributes.getString(R.styleable.CountDownView_title) != null)
            messageText = attributes.getString(R.styleable.CountDownView_messageText);
        if (attributes.getString(R.styleable.CountDownView_title) != null)
            buttonText = attributes.getString(R.styleable.CountDownView_buttonText);
...
//        setTitleSize(titleSize);
    ...
    @Override
    public void setTimes(long currentTime, long startTime, TimerState timerStateListener) {
        mTimerStateListener = timerStateListener;
...
//                        setTimerFinished(false);
    ...
    public void setTimerFinished() {
        if (mTimerStateListener != null)
            mTimerStateListener.onTimerFinishedListener();
    }
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.button) {
            if (mTimerStateListener != null) mTimerStateListener.onButtonClickedListener();
        }
    }
